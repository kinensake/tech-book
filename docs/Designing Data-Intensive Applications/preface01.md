...
1. In [Part I](part01.md), we discuss the fundamental ideas that underpin the design of data-intensive applications. We start in [Chapter 1](ch01.md) by discussing what we’re actually trying to achieve: reliability, scalability, and maintainability; how we need to think about them; and how we can achieve them. In [Chapter 2](ch02.md) we compare several different data models and query languages, and see how they are appropriate to different situations. In [Chapter 3](ch03.md) we talk about storage engines: how databases arrange data on disk so that we can find it again efficiently. [Chapter 4](ch04.md) turns to formats for data encoding (serialization) and evolution of schemas over time.
2. In [Part II](part02.md), we move from data stored on one machine to data that is distributed across multiple machines. This is often necessary for scalability, but brings with it a variety of unique challenges. We first discuss replication ([Chapter 5](ch05.md)), partitioning/sharding ([Chapter 6](ch06.md)), and transactions ([Chapter 7](ch07.md)). We then go into more detail on the problems with distributed systems ([Chapter 8](ch08.md)) and what it means to achieve consistency and consensus in a distributed system ([Chapter 9](ch09.md)).
3. In [Part III](part03.md), we discuss systems that derive some datasets from other datasets. Derived data often occurs in heterogeneous systems: when there is no one database that can do everything well, applications need to integrate several different databases, caches, indexes, and so on. In [Chapter 10](ch10.md) we start with a batch processing approach to derived data, and we build upon it with stream processing in [Chapter 11](ch11.md). Finally, in [Chapter 12](ch12.md) we put everything together and discuss approaches for building reliable, scalable, and maintainable applications in the future.
...
We have a web page for this book, where we list errata, examples, and any additional information. You can access this page at [*http://bit.ly/designing-data-intensive-apps*](http://bit.ly/designing-data-intensive-apps).
